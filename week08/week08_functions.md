...
**คำตอบ:** `value` เปลี่ยนเป็น 100 แต่ hp ยังเป็น 150 เพราะฟังก์ชันรับค่าเป็น copy จึงไม่ได้เปลี่ยนค่าต้นฉบับใน `main()`

### 2.2 `lerp` — Local Scope

**คำถาม:** ทำไม `main()` ถึงมองไม่เห็นตัวแปร `result` ทั้งที่มันถูกประกาศไว้ในไฟล์เดียวกัน?

**คำตอบ:** `result` เป็นตัวแปร local ใช้ได้แค่ภายในฟังก์ชัน `lerp()`
...
**คำตอบ:** `min` เป็นเพียงค่าที่ copy มาจาก `diceMin` จึงไม่ส่งผลต่อต้นฉบับ
...
**คำตอบ:** `current` เป็นค่าที่ copy มาจาก `score` การแก้ในฟังก์ชัน จึงไม่เปลี่ยน `score` ใน `main()`
